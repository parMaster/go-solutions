package main

import (
	"fmt"

	"golang.org/x/tour/pic"
)

func Pic(dx, dy int) [][]uint8 {

	table := make([][]uint8, dx)

	for col := range table {
		table[col] = make([]uint8, dy)
	}

	for col := range table {
		for row := range table[col] {
			table[col][row] = uint8(col + row)
		}
	}

	return table
}

func showSlice(slice [][]uint8) {
	for i := range slice {
		fmt.Println(slice[i])
	}
}

func main() {

	// fmt.Println(Pic(10, 10))
	pic.Show(Pic)
	// <img src="data:image/png;base64,">

}
